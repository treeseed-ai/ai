...
it('authenticates to the inference destination during readiness',async()=>{
	const prior={bucket:process.env.S3_BUCKET,endpoint:process.env.S3_ENDPOINT,access:process.env.S3_ACCESS_KEY,secret:process.env.S3_SECRET_KEY},put=vi.fn(async()=>({})),bytes=vi.fn(async(key:string)=>new TextEncoder().encode(key.includes('product-credential')?'treeai-product-credential-v1':'treeai-artifact-readiness-v1'));
	...
	try{const create=vi.fn(()=>({bucket:'ai-inference',put,bytes}as unknown as ArtifactStore));await verifyArtifactDestination(create);expect(create).toHaveBeenCalledWith('ai-inference','http://inference-minio:9000','inf-test','not-logged');expect(put).toHaveBeenCalledOnce();expect(bytes).toHaveBeenCalledTimes(2);}
