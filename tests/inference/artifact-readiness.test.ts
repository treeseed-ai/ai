...
it('authenticates to the named training source during readiness',async()=>{
	const root=mkdtempSync(join(tmpdir(),'treeai-artifact-source-')),path=join(root,'source.json'),list=vi.fn(async()=>({Contents:[]}));
	...
	const create=vi.fn(()=>({list}as unknown as ArtifactStore));
...
it('fails readiness when the training source rejects its credential',async()=>{
	...
	process.env.ARTIFACT_SOURCE_REGISTRY=path;
	await expect(verifyArtifactSource(()=>({list:async()=>{throw new Error('source credential rejected');}}as unknown as ArtifactStore))).rejects.toThrow('source credential rejected');
});
