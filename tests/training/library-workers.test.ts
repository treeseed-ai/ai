...
describe('library document workers',()=>{
	...
	it('gives non-root Axolotl persistent writable compiler and model caches',()=>{const compose=readFileSync('deploy/training/compose.yml','utf8'),dockerfile=readFileSync('containers/training/axolotl.Dockerfile','utf8');expect(compose).toContain('HF_HOME: /models/huggingface');expect(compose).toContain('XDG_CACHE_HOME: /models/cache');expect(compose).toContain('TORCH_HOME: /models/torch');expect(compose).toContain('TRITON_CACHE_DIR: /models/triton');expect(dockerfile).toContain('/models/triton');expect(dockerfile).toContain('chown -R worker /artifacts /models');expect(dockerfile).toContain('USER worker');});
