...
describe('library document workers',()=>{
	...
	it('invokes Marker for Markdown and structured JSON with writable persistent caches',()=>{const worker=readFileSync('workers/marker/worker.py','utf8'),compose=readFileSync('deploy/training/compose.yml','utf8'),dockerfile=readFileSync('containers/training/marker.Dockerfile','utf8');expect(worker).toContain('["marker_single",str(source),"--output_dir",str(target),"--output_format",output_format]');expect(worker).toContain('run_marker(source,markdown_target,"markdown")');expect(worker).toContain('run_marker(source,structured_target,"json")');expect(worker).not.toContain('"--mode"');expect(worker).toContain('[-12000:]');expect(worker).toContain('SECRET.sub("[REDACTED]"');expect(compose).toContain('HF_HOME: /models/huggingface');expect(compose).toContain('FONT_PATH: /models/cache/fonts/GoNotoCurrent-Regular.ttf');expect(compose).toContain('["training-artifacts:/artifacts", "training-models:/models"]');expect(dockerfile).toContain('chown -R worker /inputs /artifacts /models');});
...
with tempfile.TemporaryDirectory() as target:
 ...
 def run(command,**kwargs):
  output=pathlib.Path(command[command.index('--output_dir')+1]);output.mkdir(parents=True,exist_ok=True);kind=command[command.index('--output_format')+1]
  if kind=='markdown':(output/'document.md').write_text('# PDF Knowledge\\nParsed protocol relationships.')
  else:(output/'document.json').write_text(json.dumps({'type':'Page','page':1,'children':[{'type':'SectionHeader','text':'Protocol diagram'},{'type':'Text','text':'Authored explanation of the protocol relationship.'}]}))
