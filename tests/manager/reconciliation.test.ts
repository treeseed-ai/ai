...
describe('manager-owned platform reconciliation',()=>{
  ...
  it('provisions a distinct read-only training artifact exchange identity',()=>{
	...
	it('gates training API readiness on its effective object-store identity',()=>{
		...
		const platform=readFileSync('packages/manager/src/lifecycle/platform.ts','utf8');expect(main).toContain('trainingArtifacts(config.artifacts)');expect(main).toContain('verifyArtifacts(objects)');expect(compose).toContain("fetch('http://127.0.0.1:4780/readyz')");expect(compose).toMatch(/api:[\s\S]*healthcheck:[^\n]+start_period: 60s/u);expect(platform).toContain('training: ["postgres", "minio", "migrations", "artifact", "manager"]');expect(platform).toContain('["up", "-d", "--no-deps", "api"]');expect(platform).toContain('verifyProductReadiness(product,reconcileProduct)');
	});
	it('keeps the legacy product credential marker while inference verifies its selected repository',()=>{const compose=readFileSync('deploy/inference/compose.yml','utf8'),artifacts=readFileSync('packages/inference-api/src/artifacts.ts','utf8');expect(compose).toContain('mc pipe product-check/$$S3_BUCKET/_health/product-credential-v1');expect(artifacts).toContain("stage('destination-write'");expect(artifacts).toContain('destinationConfiguration()');});
