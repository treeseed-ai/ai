...
export function createExperienceProxy(options: ProxyOptions = {}) {
	...
	app.get("/v1/models", async (context) => {
		...
		headers.set("authorization", `Bearer ${required("AI_INFERENCE_KEY")}`); headers.delete("host");
	...
	app.all("/v1/*", async (context) => {
		...
		const headers = new Headers(context.req.raw.headers);
		headers.set("authorization", `Bearer ${agentRequest ? required("HERMES_API_KEY") : required("AI_INFERENCE_KEY")}`);
