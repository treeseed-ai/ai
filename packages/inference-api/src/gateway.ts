...
export function createInferenceGateway(input: { rawVllmUrl: string; publicModel: string; sourceModel: string; resolveModel?:(requested:string)=>Promise<string|null>; listModels?:()=>Promise<Array<{id:string;root:string}>>; resolveKey: ApiKeyResolver; fetch?: typeof fetch }) {
	const app = new Hono(); const fetcher = input.fetch ?? fetch;let active=0;const admissionPath=process.env.TREESEED_GPU_ADMISSION_FILE,statusPath=process.env.TREESEED_GPU_ACTIVITY_FILE;const admitted=()=>{try{if(admissionPath)return existsSync(admissionPath)&&JSON.parse(readFileSync(admissionPath,'utf8')).admission==='open';return true;}catch{return false;}};const status=()=>{if(!statusPath)return;mkdirSync(dirname(statusPath),{recursive:true});const temporary=`${statusPath}.${process.pid}.tmp`;writeFileSync(temporary,JSON.stringify({active,updatedAt:new Date().toISOString()}));renameSync(temporary,statusPath);};
