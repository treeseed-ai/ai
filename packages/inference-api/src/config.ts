...
export function inferenceConfig(env = process.env): InferenceConfig {
	return {
		version: env.AI_VERSION ?? '0.10.0', controlHost: env.CONTROL_HOST ?? '127.0.0.1', controlPort: numberEnv('CONTROL_PORT', 4770, env),
		inferenceHost: env.INFERENCE_HOST ?? '0.0.0.0', inferencePort: numberEnv('INFERENCE_PORT', 4771, env),
		databaseUrl: readAiDatabaseUrl('inference', env), rawVllmUrl: env.VLLM_URL ?? 'http://vllm:8000',
