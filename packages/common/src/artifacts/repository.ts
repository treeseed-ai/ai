...
import {ManagedArtifactStore} from '../storage.js';
import type {StorageCustody} from './custody.js';
...
export function resolveArtifactKey(value:string,storeId:string){if(!value.includes('://'))return artifactKey(value);const parsed=value.match(/^artifact:\/\/([^/]+)\/(.+)$/u);if(parsed){if(parsed[1]!==storeId)throw new Error('Artifact belongs to a different store.');return artifactKey(parsed[2]!);}throw new Error('Artifact URI is not accepted by this store.');}

export class FilesystemArtifactRepository implements ArtifactRepository{
	readonly root:string;
	constructor(readonly storeId:string,root:string){this.root=resolve(root);mkdirSync(this.root,{recursive:true,mode:0o750});if(lstatSync(this.root).isSymbolicLink())throw new Error('Artifact root cannot be a symlink.');}
	uri(key:string){return artifactUri(this.storeId,key);}
	key(uri:string){return resolveArtifactKey(uri,this.storeId);}
...
export class R2ArtifactRepository implements ArtifactRepository{
	private readonly storage:ManagedArtifactStore;
	constructor(readonly storeId:string,custody:StorageCustody){this.storage=new ManagedArtifactStore(storeId,custody);}
	uri(key:string){return artifactUri(this.storeId,key);}
	key(uri:string){return resolveArtifactKey(uri,this.storeId);}
	...
	async list(prefix=''){const keys=await this.storage.keys(prefix);const objects:RepositoryObject[]=[];for(const key of keys)objects.push(await this.head(key));return objects;}
}
