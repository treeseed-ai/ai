...
export class PostgresJobRepository implements JobRepository {
	...
	async complete(id: string, worker: string, resultManifest?: string) {
		const result = await this.pool.query(`UPDATE jobs SET state=CASE WHEN cancellation_requested THEN 'cancelled' ELSE 'succeeded' END,progress=CASE WHEN cancellation_requested THEN progress ELSE 1 END,result_manifest=$3,lease_owner=null,lease_expires_at=null,updated_at=now() WHERE id=$1 AND lease_owner=$2`, [id, worker, resultManifest ?? null]);
