...
export function profileCompatible(profile: Pick<MachineProfile,"settings"> | null) { return Boolean(profile && profile.settings.loraRank === supportedLoraRank); }
export function qualificationStatus(run?: Runner) { const current = observedFingerprint(run), currentDigest = digest(current), activeId = setting<string | null>("qualification.activeProfile", null), knownGoodId = setting<string | null>("qualification.knownGoodProfile", null), active = activeId ? readProfile(activeId) : null; return { status: active && profileCompatible(active) ? "ready" : "warning", fingerprint: current, fingerprintDigest: currentDigest, baselineRequired: setting("qualification.fingerprint", "") !== currentDigest || !profileCompatible(active), sustainedTrainingProfile:observedTrainingProfile(), activeProfile: active, knownGoodProfile: knownGoodId ? readProfile(knownGoodId) : null }; }
...
export function scoresComparable(left:Pick<MachineProfile,"scorePolicy"|"fingerprintDigest">,right:Pick<MachineProfile,"scorePolicy"|"fingerprintDigest">){return Boolean(left.scorePolicy&&left.scorePolicy===right.scorePolicy&&left.fingerprintDigest===right.fingerprintDigest);}
export function activateProfile(id: string) { const next = readProfile(id); if (!next || next.state === "rejected" || !profileCompatible(next)) throw new Error("Only a verified, compatible passing profile may be activated."); const priorId = setting<string | null>("qualification.activeProfile", null), prior = priorId ? readProfile(priorId) : null; if (prior && profileCompatible(prior)&&scoresComparable(next,prior)&&next.score <= prior.score) return { changed: false, reason: "not_strictly_better", activeProfile: prior }; if (prior) { prior.state = "known-good"; writeProfile(prior); setSetting("qualification.knownGoodProfile", prior.id); } next.state = "active"; writeProfile(next); setSetting("qualification.activeProfile", next.id); setSetting("qualification.fingerprint", next.fingerprintDigest); event("qualification.profile-activated", { profileId: next.id, priorProfileId: prior?.id }); return { changed: true, activeProfile: next }; }
