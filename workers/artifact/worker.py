...
def dataset(job):
    ...
    target=ROOT/"datasets"/f"{job['jobId']}.json";target.parent.mkdir(exist_ok=True);target.write_bytes(canonical(manifest));return{"resultManifest":upload(target,f"datasets/{job['jobId']}.json")}
...
def experience_register(job):
    manifest=validate_experience(job["input"].get("manifest",{}));target=ROOT/"experience"/f"{manifest['batchId']}.json";target.parent.mkdir(exist_ok=True);target.write_bytes(canonical(manifest));return{"resultManifest":upload(target,f"experience/{manifest['batchId']}.json"),"trajectoryCount":len(manifest["trajectories"])}
def experience(job):
