...
def verified_manifest(repository,uri,key_path):
    value=json.loads(repository.bytes(uri));signature=value.pop("signature",None)
...
def relative_object(manifest,item):
    ...
    return relative
def object_bytes(repository,item):
    ...
    return data
def canonical_peft_objects(manifest,repository):
    model=None;config=None
    for item in manifest["objects"]:
        relative=relative_object(manifest,item);data=object_bytes(repository,item)
...
def compose(job,repository,key_path,upload,sign):
    ...
    manifests=[verified_manifest(repository,uri,key_path) for uri in uris]
    ...
    parents=[canonical_peft_objects(manifest,repository) for manifest in manifests];models=[item[0] for item in parents]
