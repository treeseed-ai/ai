...
RUN chmod -R a+rX /app && useradd --system --uid 10001 worker && mkdir -p /artifacts/training /models/huggingface /models/cache /models/torch /models/triton && chown -R worker /artifacts /models
USER worker
