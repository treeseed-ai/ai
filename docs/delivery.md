...
Release candidates use the manually dispatched **Publish protected TreeAI release candidate** workflow from `staging`. Tags omit a `v` prefix and follow `0.10.0-rc1`; Debian versions use `0.10.0~rc1-1` so the final `0.10.0-1` sorts newer. APT signing material is held only in the protected `staging` environment and differs from the stable key. The workflow never builds or publishes development containers. It verifies the latest stable image manifest and signatures, reuses unchanged production digests, and records whether the candidate is `package-only` or `local-images-required`.
